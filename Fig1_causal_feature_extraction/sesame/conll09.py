...
CAUSE_WORDS = ["because of", "because", "bring about", "bring on", "bring", "causative","cause", "cause", "consequence", "consequent", "consequential", "dictate","due to", "for", "force", "give rise", "induce", "lead (to)", "leave", "legacy", "make", "mean", "motivate", "precipitate", "put", "raise", "reason", "render", "responsible", "result (in)", "result", "resultant", "resulting", "see", "send", "since", "so", "sway", "wreak", "cause"]
...
class CoNLL09Element:
    ...
    def get_lu_str(self):
	return LUDICT.getstr(self.lu)


    def get_role_str(self):
	if self.is_arg:
	    return FEDICT.getstr(self.role)
	else:
	    return ""


    def get_token_str(self):
	return VOCDICT.getstr(self.form)
...
class CoNLL09Example(FrameSemParse):
    ...
    def is_cause_frame(self):
	for e in self._elements:
	    if e.get_lu_str() in CAUSE_WORDS:
		return True
	return False    
	...
	cause_word = ""
	for e in self._elements:
	    if e.get_role_str() == "Cause":
		cause.append(e.get_token_str())
	    elif e.get_role_str() == "Effect":
		effect.append(e.get_token_str())
	    elif e.get_lu_str() in CAUSE_WORDS:
		cause_word = e.get_token_str()
	return " ".join(cause) + "|" + cause_word + "|" + " ".join(effect) 
