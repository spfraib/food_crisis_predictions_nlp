...
def grangercausalitytests(x, x_test, mxlg, autolag=None, alpha=0.0001, max_iter=1e5, addconst=True, verbose=True, glm=False):
    ...
    dta_test = lagmat2ds(x_test, mxlg, trim='both', dropex=1)

    #add constant
    if addconst:
        dtajoint = add_constant(dta[:, 1:], prepend=False)
        dtajoint_test = add_constant(dta_test[:, 1:], prepend=False)
    else:
        raise NotImplementedError('Not Implemented')

    # Run Lasso on all variables
    if glm:
        glm_binom = sm.GLM(dta[:, 0], dtajoint[:,1:], family=sm.families.Binomial())
        glm_res = glm_binom.fit()
        pred = glm_binom.predict(glm_res.params, dtajoint_test[:,1:])
        result = glm_res.params
    else:
        lassoreg = Lasso(alpha=alpha,normalize=True, max_iter=max_iter)
        lassoreg.fit(dtajoint[:,1:], dta[:, 0])
        pred = lassoreg.predict(dtajoint_test[:,1:])
        result = lassoreg.coef_	
    actual = dta_test[:,0]
    errors = [(i-j)*(i-j) for i, j in zip(actual, pred)]
    step_size = 4
    window_size = 30
    avg_errors=[]
    ...
    for i in range(len(actual)):
        binarized.append(1 if actual[i] > 0 else 0)
        binarized_pred.append(1 if pred[i] > 0 else 0)
    #binarized = actual.apply(lambda x: 1 if x > 0 else 0)
    try:
        auc = roc_auc_score(binarized, binarized_pred)
    except:
        auc = 'NA'    
    rmse = np.mean(errors)
    ...
    for (i,x) in non_zeros:
        ...
        if k not in non_zero_vars or abs(x) > abs(best_vars[k]):
            ...
            best_vars[k] = x
    return (auc, rmse , non_zero_vars, best_vars, pred)
