...
optpr = OptionParser()
optpr.add_option("--mode", dest="mode", type="choice", choices=["train", "test", "refresh", "filter", "extract"], default="train")
...
model_dir = "{}/logs/{}/".format(base_dir, options.model_name)
...
elif options.mode in ["filter", "extract"]:
    assert options.raw_input is not None
    instances, _, _ = read_conll(options.raw_input)
    if options.mode == "filter":
	out_conll_file = "{}filtered-frames.conll".format(output_file_dir)
    else:
	out_conll_file = "{}args-tuples.csv".format(output_file_dir)
else:
...
def print_as_conll(filteredexamples):
    with codecs.open(out_conll_file, "w", "utf-8") as f:
        for g in filteredexamples:
            result = g.get_str() + "\n"
            f.write(result)
        f.close()


def print_args(filteredexamples):
    with codecs.open(out_conll_file, "w", "utf-8") as f:
	for g in filteredexamples:
	    g_args = g.get_args() + "\n"
	    f.write(g_args)
...
if options.mode in ["train", "refresh"]:
    ...
    for instance in instances:
        predictions.append(instance)
    sys.stderr.write("Printing output in CSV format to {}\n".format(out_conll_file))
    print_args(predictions)
    sys.stderr.write("Done!\n")
